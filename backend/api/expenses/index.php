...
if ($method === 'GET') {
  if ($id > 0) {
    $s = $db->prepare('SELECT * FROM expenses WHERE id=? AND company_id=?');
  ...
  $total=$db->prepare('SELECT COUNT(*) FROM expenses WHERE company_id=?'); $total->execute([$co]);
  $rows=$db->prepare('SELECT * FROM expenses WHERE company_id=? ORDER BY id DESC LIMIT '.$per.' OFFSET '.$offset); $rows->execute([$co]);
  paged($rows->fetchAll(),(int)$total->fetchColumn(),$page,$per);
}
if ($method === 'POST') {
  $b=body();
  // Build dynamic insert from body (sanitized)
  $allowed = array_keys($db->query('DESCRIBE expenses')->fetchAll(PDO::FETCH_KEY_PAIR));
  ...
  $sql='INSERT INTO expenses ('.implode(',',$fields).') VALUES ('.implode(',',array_fill(0,count($fields),'?')).')';
  $db->prepare($sql)->execute($vals); $newId=$db->lastInsertId();
  $row=$db->prepare('SELECT * FROM expenses WHERE id=?'); $row->execute([$newId]); ok($row->fetch(),'Created',201);
}
if ($method === 'PUT' && $id > 0) {
  $b=body(); $sets=[]; $vals=[];
  $allowed = array_keys($db->query('DESCRIBE expenses')->fetchAll(PDO::FETCH_KEY_PAIR));
  ...
  $vals[]=$id; $vals[]=$co;
  $db->prepare('UPDATE expenses SET '.implode(',',$sets).' WHERE id=? AND company_id=?')->execute($vals);
  ok(['id'=>$id],'Updated');
}
if ($method === 'DELETE' && $id > 0) {
  $db->prepare('DELETE FROM expenses WHERE id=? AND company_id=?')->execute([$id,$co]); ok(null,'Deleted');
}
